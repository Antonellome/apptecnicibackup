...
const NuovoRapportinoPage: React.FC = () => {
    ...
    // --- DATI ORDINATI PER I MENU ---
    const sortedTipiGiornata = useMemo(() => [...tipiGiornata].sort((a, b) => a.nome.localeCompare(b.nome)), [tipiGiornata]);
    const sortedTecnici = useMemo(() => [...tecnici].sort((a, b) => a.cognome.localeCompare(b.cognome)), [tecnici]);
        ...
        try {
            if (isPeriodo && !isEditMode && dataInizio && dataFine) {
                ...
                days.forEach(day => {
                    ...
                    const rapportinoData = { data: Timestamp.fromDate(day), tecnicoId: loggedInTecnicoId, tipoGiornataId: tipoGiornataId, isLavorativo: tipoSelezionato.lavorativo, oreLavoro: 0, isManuale: false, partecipanti: [loggedInTecnicoId], createdAt: Timestamp.now(), createdBy: loggedInTecnicoId };
            ...
            <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: 800, mx: 'auto' }}>
                <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, maxHeight: '90vh', overflowY: 'auto' }}>
                    ...
                    <Box component="form" noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mt: 2 }}>
                        {!isEditMode && <Alert severity="info" sx={{ mt: 1, display: 'flex', alignItems: 'center' }}><FormControlLabel control={<Switch checked={isPeriodo} onChange={e => setIsPeriodo(e.target.checked)} disabled={isEditMode || isSaving} />} label="Inserisci per un periodo di più giorni" /></Alert>}
                        ...
                        {isLavorativo && !isPeriodo && (
                            <>
                                ...
                                {!isManuale ? (
                                    <Grid container spacing={2}>
                                        <Grid item xs={12} sm={4}><FormControl fullWidth><InputLabel>Inizio</InputLabel><Select value={oraInizio || ''} label="Inizio" onChange={e => setOraInizio(e.target.value)} disabled={isReadOnly || isSaving}>{timeOptions.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}</Select></FormControl></Grid>
                                        <Grid item xs={12} sm={4}><FormControl fullWidth><InputLabel>Fine</InputLabel><Select value={oraFine || ''} label="Fine" onChange={e => setOraFine(e.target.value)} disabled={isReadOnly || isSaving}>{timeOptions.map(t => <MenuItem key={t} value={t}>{t}</MenuItem>)}</Select></FormControl></Grid>
                                        ...
                                        <Grid item xs={12}><TextField label="Totale Ore Calcolato" value={(Number(oreLavoro) || 0).toFixed(2)} fullWidth disabled /></Grid>
                                    </Grid>
                                ) : (
                                ...
                                <Autocomplete multiple options={sortedTecnici.filter(t => t.id !== loggedInTecnicoId)} getOptionLabel={o => `${o.cognome} ${o.nome}`} value={sortedTecnici.filter(t => altriTecniciIds.includes(t.id))} onChange={(_, nv) => setAltriTecniciIds(nv.map(v => v.id))} renderInput={params => <TextField {...params} label="Altri Tecnici" />} disabled={isReadOnly || isSaving} />
                                <FormControl fullWidth><InputLabel>Veicolo</InputLabel><Select value={veicoloId || ''} label="Veicolo" onChange={e => setVeicoloId(e.target.value)} disabled={isReadOnly || isSaving}><MenuItem value=""><em>Nessuno</em></MenuItem>{sortedVeicoli.map(v => <MenuItem key={v.id} value={v.id}>{`${v.targa} - ${v.descrizione}`}</MenuItem>)}</Select></FormControl>
                                <FormControl fullWidth><InputLabel>Nave</InputLabel><Select value={naveId || ''} label="Nave" onChange={e => setNaveId(e.target.value)} disabled={isReadOnly || isSaving}><MenuItem value=""><em>Nessuna</em></MenuItem>{sortedNavi.map(n => <MenuItem key={n.id} value={n.id}>{n.nome}</MenuItem>)}</Select></FormControl>
                                <FormControl fullWidth><InputLabel>Luogo</InputLabel><Select value={luogoId || ''} label="Luogo" onChange={e => setLuogoId(e.target.value)} disabled={isReadOnly || isSaving}><MenuItem value=""><em>Nessuno</em></MenuItem>{sortedLuoghi.map(l => <MenuItem key={l.id} value={l.id}>{l.nome}</MenuItem>)}</Select></FormControl>
                        ...
                        <Grid container spacing={2} justifyContent="flex-end" sx={{ mt: 2 }}>
                            <Grid item><Button variant="outlined" size="large" onClick={() => navigate('/rapportini')} disabled={isSaving}>{isReadOnly ? 'Indietro' : 'Annulla'}</Button></Grid>
                            {!isReadOnly && <Grid item><Button variant="contained" color="primary" size="large" onClick={handleSubmit} disabled={isSaving || (isPeriodo && (!dataInizio || !dataFine))}>{isSaving ? <CircularProgress size={24} /> : 'Salva'}</Button></Grid>}
                        </Grid>
                    </Box>
                </Paper>
            </Box>
