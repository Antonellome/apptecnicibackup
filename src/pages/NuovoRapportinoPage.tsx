...
import {
    ...
    Switch, FormControlLabel, Autocomplete, Button, CircularProgress, Grid
} from '@mui/material';
...
import { TipoGiornata, Tecnico, Veicolo, Nave, Luogo } from '../models/definitions'; // Importa le definizioni corrette
...
const NuovoRapportinoPage: React.FC = () => {
    ...
    // Ora tipiGiornata sarà correttamente tipizzato grazie all'import
    ...
    useEffect(() => {
        if (!isEditMode || !reportId || !collections || tipiGiornata.length === 0) {
            setIsLoading(false);
            return;
        }
        ...
        loadReportData();
    }, [isEditMode, reportId, navigate, collections, tipiGiornata, loggedInTecnicoId]);
    ...
    // --- GESTIONE CAMBIO TIPO GIORNATA ---
    const handleTipoGiornataChange = (id: string) => {
        setTipoGiornataId(id);
        const tipoSelezionato = tipiGiornata.find(t => t.id === id);

        if (tipoSelezionato) {
            setIsLavorativo(tipoSelezionato.lavorativo === true);
            // QUESTA LOGICA ORA FUNZIONERA' PERCHE' L'APP CONOSCE `isPeriodo`
            setIsPeriodo(tipoSelezionato.isPeriodo === true && !isEditMode);
        } else {
        ...
        try {
            if (isPeriodo && !isEditMode && dataInizio && dataFine) {
                ...
                days.forEach(day => {
                    ...
                    batch.set(newReportRef, { data: Timestamp.fromDate(day), tecnicoId: loggedInTecnicoId, tipoGiornataId, oreLavoro: 0, isLavorativo: false, isManuale: false, partecipanti: [loggedInTecnicoId], createdAt: Timestamp.now(), createdBy: loggedInTecnicoId });
                });
                await batch.commit();
                ...
                <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, maxHeight: '90vh', overflowY: 'auto' }}>
                    ...
                    <Box component="form" noValidate sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mt: 2 }}>
                        ...
                        <Grid container spacing={2} justifyContent="flex-end" sx={{ mt: 2 }}>
                            <Grid item><Button variant="outlined" size="large" onClick={() => navigate('/reports')} disabled={isSaving}>{isReadOnly ? 'Indietro' : 'Annulla'}</Button></Grid>
