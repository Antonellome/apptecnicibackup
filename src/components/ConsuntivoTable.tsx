...
const calculateTotals = (reports: EnrichedReport[]): Totals => {
    ...
    reports.forEach(report => {
        ...
        if (tipoGiornataInfo) {
            ...
            // FIX: La proprietà `costoOrario` non esiste su `TipoGiornata`.
            // Ci basiamo solo su `tariffa`, assumendo che sia una tariffa fissa come da logica originale.
            const tariffa = (tipoGiornataInfo as any).tariffa;
            if (typeof tariffa === 'number' && tariffa > 0) {
                costoGiornaliero = tariffa;
            }
